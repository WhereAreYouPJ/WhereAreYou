...
//    private val userIdRule = Regex("^[a-z][a-z0-9]{3,9}\$")
    // 영문 소문자와 숫자만 사용하여, 영문 소문자로 시작하는 5~12자의 텍스트
    private val userIdRule = Regex("^[a-z][a-z0-9]{4,11}\$")
    // 영문 대/소문자로 시작하는 6~20자의 영문 대/소문자, 숫자
    // 영문 대문자, 소문자, 숫자를 최소 하나 이상씩 포함
    private val passwordRule = Regex("^(?=[A-Za-z])(?=.*[A-Z])(?=.*[a-z])(?=.*[0-9])[A-Za-z0-9]{6,20}\$")
        // []@[].[] 형식
